...
function inputhandleFunctionality() {

  let changeOverLay = document.querySelector('#searchinput');


  changeOverLay.addEventListener('focus', function () {
  ...
  changeOverLay.addEventListener('blur', function () {
  ...
  changeOverLay.addEventListener('input', () => {

    var filteredArray = arr.filter((obj) => {


      return obj.name.startsWith(changeOverLay.value)
    ...
    filteredArray.forEach(function (obj) {
      cullter += `<div class="res flex px-8 py-3">
            <i class="ri-search-line font-semibold mr-5"></i>
            <h3 class=" suggestions font-semibold">${obj.name}</h3>
        </div>` ;
    ...
    document.querySelector('.searchdata').innerHTML = cullter;
